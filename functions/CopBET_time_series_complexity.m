...
for ses = 1:height(in)
    ...
    %First Hilbert transform the BOLD ts and take the absolute values
    abs_hts = abs(hilbert(ts));
    mean_bold = mean(abs_hts);
    bin_abs_hts = (abs_hts > mean_bold);
    
    % create random ts by shuffling every regional time-series
    random_ts = nan(size(abs_hts));
    for roi = 1:size(ts,2)
        % Shuffle Hilbert ts
        random_ts(:,roi) = abs_hts(randperm(length(abs_hts(:,roi))),roi);
    end
    M_rand = (random_ts > mean_bold); %mean should be identical
    
    C = nan(size(ts,2),1);
    C_rand = nan(size(ts,2),1);
    if strcmp(LZtype,'LZ78temporal')
        longts = bin_abs_hts(:); 
        C=cpr(char(longts+'0'));
        long_rand = M_rand(:);
        C_rand=cpr(char(long_rand+'0'));
    elseif strcmp(LZtype,'LZ78spatial')
        %transpose
        bin_abs_hts = bin_abs_hts';
        M_rand = M_rand';
    ...
    elseif strcmp(LZtype,'LZ76temporal')
        longts = bin_abs_hts(:); 
        [C, H] = calc_lz_complexity(longts, 'exhaustive', 1);
        long_rand = M_rand(:);
        [C_rand, H_rand] = calc_lz_complexity(long_rand, 'exhaustive', 1);
        
    elseif strcmp(LZtype,'LZ76spatial')
        %transpose
        bin_abs_hts = bin_abs_hts';
        M_rand = M_rand';
